...
if SDWAN_IP is None or SDWAN_USERNAME is None or SDWAN_PASSWORD is None:
    print("SDWAN details must be set via environment variables before running.")
    print("   export SDWAN_IP=10.0.0.123")
...
@click.group()
def cli():
    """Command line tool for deploying templates to SDWAN.
    """
    pass
...
def attach(template, target, hostname, sysip, loopip, geip, siteid):
    """Attach a template with SDWAN.

        Provide all template parameters and their values as arguments.

        Example command:

          ./sdwan.py attach --template TemplateID --target TargetID --hostname rst.vedge.com 
